...
func fiberRoute(app *fiber.App) {
	...
	apiGet.Get("/sqlqh/:sql_no", sqlqh)  // Query from sql_no (or sql) ==> host:/port/api/key/get/sqlq/:sql_no/:sql_no?sql=...
	...
	apiGet.Get("/url", urlJson)          // get data from url response ==> host:port/api/key/get/url/...
	...
	apiFile.Post("/fileupdt", fileUploadTemp)
	apiFile.Post("/fileupd", fileUpload)
